...
| Decky plugin     | `~/homebrew/plugins/hiberdecky` — QAM moon tile, calls script, 30s post-resume cooldown                                                                 |
| Double-trigger hardening | A double-tap used to spawn two `claw-hibernate.sh` runs; the 2nd (frozen mid-run, restored from the image on resume) fired its own `echo disk` → **second hibernate right after waking from the first**. Fixed 2026-07-09 at 3 layers: (1) **script `flock -n` on `/run/claw-hibernate.lock`** held for the whole run — the real cross-source backstop (covers Decky, HHD wrapper, auto-hibernate); (2) **`main.py` `self._busy` re-entrancy guard** (race-free in the single-threaded loop); (3) **frontend disables the button + shows "Hibernating…" + a "Entering hibernation…" toast** — the silent click→dark-screen gap was what invited the double-tap. Cooldown cut 60s→30s to match the script's sleep-inhibitor guard. Frontend needs `npm run build` (rollup) → deploy `dist/index.js`; reload via `systemctl restart plugin_loader`. |
...
| Item            | Location / detail                                                                                                                           |
|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------|
| Sleep hook      | `/etc/systemd/system-sleep/50-msi-claw-s2idle-power.sh` — BT block, ZT stop, fingerprint USB unbind, conditional gamepad rebind only if raw HID is missing or the `usb 3-9` `-75` IRQ storm is present, battery kick, WiFi fix. Repo copy tracked at `system/system-sleep/50-msi-claw-s2idle-power.sh`. |
| Suspend drop-in | `systemd-suspend.service.d/msi-claw-s2idle.conf`                                                                                            |
| Audio fix       | `99-audio-resume-fix.disabled` — **disabled**                                                                                               |
| Fans loud in sleep-when-hot | **No OS-side fix — not fixable.** EC owns the fan curve (`msi_wmi_platform`: `pwm*_enable` only accepts 1/2, **no `pwm*` duty file**, `auto_point*_pwm` writes don't stick, manual mode has no effect). In s2idle the EC keeps its curve live and the platform never reaches S0ix (wakeup-storm keeps the SoC warm — see [firmware floor]), so fans stay audible until it passively cools below ~50 °C. A `platform_profile=low-power`-on-suspend workaround was **tried and reverted 2026-07-09** — only ~15% quieter and not even reliably applied during s2idle; user confirmed still noisy. **Only way to get silent sleep = hibernate** (full power-off). |
